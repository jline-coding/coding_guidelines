...
- Không sử dụng chữ in hoa cho tag, class name hoặc ID. Khi nối các từ trong tên class, sử dụng dấu gạch ngang đơn `-` để nối từ（ví dụ：`.c-global-nav`, `.p-news-list`）, dấu gạch dưới kép `__` để phân tách Element, dấu gạch ngang kép `--` để phân tách Modifier（chi tiết xem phần「Quy tắc đặt tên Class」）
...
- Không lồng Element trong Element (tên class chỉ có 1 cấp `__`). Nếu DOM lồng sâu, hãy tách thành Block mới
- Trong Sass, không nest selector quá **3 cấp**

###### Quy tắc nest — ví dụ chi tiết

**① Tên class: KHÔNG lồng Element trong Element**

```css
/* ❌ SAI: lồng Element trong Element (2 cấp __) */
.p-news__list__item {}
.p-news__item__title {}
.c-card__body__text {}

/* ✅ ĐÚNG: chỉ 1 cấp __ — tất cả Element đều thuộc trực tiếp Block */
.p-news__list {}
.p-news__item {}
.p-news__title {}
.c-card__body {}
...
**② Selector Sass: KHÔNG nest quá 3 cấp**

```scss
/* ❌ SAI: nest 4 cấp trở lên */
.p-news {
  .p-news__list {
    .p-news__item {
      .p-news__title {       /* cấp 4 → quá sâu */
        color: #333;
      }
    }
  }
...
/* ✅ ĐÚNG: viết phẳng hoặc tối đa 3 cấp */
.p-news {}
.p-news__list {}
.p-news__item {}
.p-news__title {}
...
**③ Khi DOM lồng sâu: tách thành Block mới**

```html
<!-- ❌ SAI: cố gắng dồn hết vào 1 Block, tên class rất dài -->
<div class="p-news">
   <ul class="p-news__list">
      <li class="p-news__item">
         <div class="p-news__item__meta">        <!-- lồng Element -->
            <span class="p-news__item__meta__date"></span>  <!-- quá sâu -->
         </div>
      </li>
   </ul>
</div>
