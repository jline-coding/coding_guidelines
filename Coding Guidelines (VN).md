...
- Không sử dụng chữ in hoa cho tag, class name hoặc ID. Khi nối các từ trong tên class, sử dụng dấu gạch ngang đơn `-` để nối từ（ví dụ：`global-nav`, `news-list`）, dấu gạch dưới kép `__` để phân tách Element, dấu gạch ngang kép `--` để phân tách Modifier（chi tiết xem phần「Quy tắc đặt tên Class」）
