...
- タイプセレクタ（要素名）+クラス名で記述しない。
  - bodyは除く  
    ...
    .c-global-nav{}
    ```
- 原則として、モジュール同士をネストするような**子孫セレクタによるスタイル指定（例: `.p-news .c-button {}`）は禁止**とする。
  - 理由: コンポーネントの独立性を損ない、詳細度（Specificity）が不要に高まるためFLOCSS/BEMの設計思想に反する。特定の場所でスタイルを変えたい場合は、Modifierクラス（例: `.c-button--news`）を使用すること。
  - **※例外として使用可能なケース:**
    1. **状態変化（State）:** JSなどで付与される `.is-active` や `.is-open` などの状態クラスを伴う場合。
    2. **CMS出力コンテンツ:** WordPressの `the_content()` など、BEMクラスを持たない生のHTMLタグが出力される箇所。親クラスで囲い、子タグを直接指定する。

    ```scss
    /* ❌ NG: モジュール同士のネスト、または冗長な指定（詳細度が上がる） */
    .p-news .c-button { ... } /* .c-buttonの独立性が失われる */
    .p-card .p-card__title { ... } /* 冗長な指定。SCSSの「&」を使うか、直接指定するべき */

    /* ✅ OK: SCSSの「&」を使った正しいBEMの書き方（コンパイル後も詳細度0,1,0を保つ） */
    .p-card {
      &__title { ... } /* コンパイル後: .p-card__title */
      &__image { ... } /* コンパイル後: .p-card__image */
    }

    /* ✅ OK: ネストの代わりとなるModifierの利用 */
    .c-button--news { ... }

    /* ✅ OK (例外 1): 状態変化 (.is-) の場合 */
    .c-button.is-active { ... } /* マルチクラス */
    .p-menu.is-open .p-menu__item { ... } /* 状態の影響範囲を限定するためのネスト */

    /* ✅ OK (例外 2): WordPressなどの動的出力コンテンツに対する指定 */
    .p-entry-content p { ... }
    .p-entry-content h2 { ... }
    ```
...
- JavaScriptで動作するインタラクティブな要素（モーダルを開く、タブ切り替えなど）や、フォームの送信ボタンに使用する。
- 判断基準：クリックしても**URLが変わらない動作**であること。

#### `<a>`タグについて

- ページ遷移を伴う場合は`<a>`タグを使用する（**URLが変わるもの**はすべて`<a>`）。
- TOPへ戻るボタンやエントリーボタンの構築には必ず`<a>`を使用し、`<button>`は使用しない。
- aタグのクリック領域はボタン全体にかかるようにする。
- パスの記述は原則相対パスを使用する。
- 外部にリンクにする際、target="_blank"を使用する際はrel="noopener noreferrer" もあわせて記述する。  
...
手作業での抽出や外部ツール（WinMergeなど）によるミスを防ぐため、現在のテンプレートは**差分ファイルの自動抽出（Auto Diff Extraction）**を備えた強力なリリース機能を搭載しています。

1. 修正を全てコミットした後、新しいタグを作成し（例：`git tag v1.0.1`）、GitHubにプッシュします（`git push origin v1.0.1`）。
2. GitHub Actionsが自動的にビルドを実行し、不要なファイル（`node_modules`や`.scss`など）を除外します。
3. **自動抽出：** システムが自動的に前回のRelease版をダウンロードし、新しいビルドと比較して、変更・追加されたファイルのみを正確に抽出して自動でZIP化します。
4. 開発者またはPMは、GitHubの **Releases** ページにアクセスし、自動生成された以下の2つの `.zip` ファイルをダウンロードして納品するだけです。
   - 📦 **フルバージョン：** `{プロジェクト名}_{tag_name}.zip`（全てのクリーンなデータ）
   - 📦 **差分データ：** `{プロジェクト名}_diff_data_{tag_name}.zip`（前回の納品から変更・追加されたファイルのみ。漏れのリスクはゼロです）

> ⚠️ **顧客からデータを受け取った際の重要注意事項：**
> GitHub Actionsが差分ファイルを比較・抽出するには、基準となるベースラインが必要です。そのため、顧客から既存のソースコードを受け取り、初めてGitにプッシュした直後に、**コードの修正を始める前に必ずタグ（例：`v1.0.0`）を作成し、GitHubにプッシュしてください。** 修正後に初めてタグを作成した場合、システムは比較対象の古いバージョンを見つけられず、差分データ（Diff）を生成できません。
