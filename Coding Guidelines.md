...
手作業での抽出によるミスを防ぐため、現在のテンプレートではGitHubの自動リリース機能を活用し、コンパイル済みのファイル（`.css`, `.webp`など）を確実に取得します。
...
3. 開発者またはPMは、GitHubの **Releases** ページにアクセスし、自動生成された `.zip` ファイルをダウンロードします。
4. **差分データ（Diff Data）の抽出方法：**
   - Releasesページから、**修正前（古いバージョン）**のZIPファイルと**修正後（新しいバージョン）**のZIPファイルを両方ダウンロードします。
   - 両方を解凍し、比較ツール（WinMergeなど）を使用して2つのフォルダを照合します。
   - ツールが変更されたファイルを正確にハイライト表示するので、それらのファイルをコピーして差分納品用のフォルダにまとめます。この方法により、ビルド済みファイルの納品漏れを完全に防ぐことができます。
